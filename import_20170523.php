	...
	*/
	...
	switch ($table) {
		...
		case 'brt':   
			...
			//brt    
				if($db->query("INSERT INTO " . DB_PREFIX . $table . " SET Spedizione = '" . $db->escape($row['E']) . "', DescrizioneEvento = '" . $db->escape($row['N']) . "', Note = '" . $db->escape($row['O']) . "', LinkBrt = '" . $row['P'] . "', Evento = '" . $row['L'] . "', DataEvento = '" . date("y-m-d", strtotime($row['J'])) . "', File = '" . (int)$input_file_id . "'")){     
					$add = true;    
				} else {
						$error = true;     
						$log->write(' ERROR: Unknown error during add information to "' . DB_PREFIX . $table  . '" table.');    
		...
		case 'ric':
			...
			$virtualRic=$ricPre+sizeof($rows);
			foreach ($rows as $row_number => $row){
				//grep only digits and letters like A or B
            	if(preg_match("/[0-9]/i", $row['A']) == 1) {
                	// string only contain the a to z , A to Z, 0 to 9
                    $str = preg_replace("/[^A-B0-9]+/", "", $row['A']);
                    $str='#'.$str;
                   }                  
...
$time=round($time,2);
?><h1><?php echo date("Y-m-d H:i:s", strtotime('+2 hours')) . " Import completed in $time seconds\n";?></h1>
