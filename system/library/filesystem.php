...
class Filesystem {
    ...
    public function __construct($registry)
    {
		...
		$this->simfdb = new DirectoryIterator(DIR_IN);
		if($types){
			$this->simfdb = new RegexIterator($this->simfdb, '/^SIMFDB_.*/');
		}
		
		$this->dbpo = new DirectoryIterator(DIR_IN);
		if($types){
			$this->dbpo = new RegexIterator($this->dbpo, '/^DBPO_.*/');
		}
		
		$this->pro = new DirectoryIterator(DIR_IN);
		if($types){
			$this->pro = new RegexIterator($this->pro, '/^PRO_.*/');
		}
		
		$this->brt = new DirectoryIterator(DIR_IN);
		if($types){
			$this->brt = new RegexIterator($this->brt, '/^BRT_.*/');
		}
	...
	// Get an array of all file named BRT
	...
	// Mario 20170806
    // Get an array of all file named STK
	...
	// Get an array of all file named DBPO
	public function getDBPO()
    {
		return $this->dbpo;
    }
	
	// Get an array of all file named PRO
