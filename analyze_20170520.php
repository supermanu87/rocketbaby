...
$con = mysqli_connect("127.0.0.1","root","huNsB!-1EraS","rocketbaby") or die("Some error occurred during connection " . mysqli_error($con));
...
if (mysqli_multi_query($con, "CALL sp_sku_dashboard();")) {
   do {
       if ($result = mysqli_store_result($con)) {
           //
       }
...
//Start Dashboard excel creation
...
if($db->query("INSERT INTO " . DB_PREFIX . $db_input_file . " SET FileName = '" . $export_filename . "', Date = '" . $dashboard_date . "'")){
...
//Start Dashboard STK excel creation 20170911 GDS


$result = $db->query("SELECT id, ArtDescrizione, ArtCodice, QuantitaDisponibile, Richieste, DisponibileDopoAllocazione from " . DB_PREFIX . $db_stk_dashboard );
...
$email->AddAddress( 'beatrice@rocketbaby.it' );
$email->AddAddress( 'supply@rocketbaby.it' );
$email->addBcc("giovanni.dsanto@gmail.com");
